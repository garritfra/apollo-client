...
export class StoreWriter {
  ...
  private processSelectionSet({
    dataId,
    // We pass in an id from policies.identify when processing fragment
    // selection sets
    id,
    result,
    selectionSet,
    ...
    let keyObject: StoreObject | undefined;
    if (id === void 0) {
      [id, keyObject] = policies.identify(
        result,
        selectionSet,
        context.fragmentMap,
      );
    ...
    selections.forEach(selection => {
      ...
      if (isField(selection)) {
        ...
        if (fragment &&
          ...
          const incomingValue = this.processSelectionSet({
            dataId,
            id,
            result,
          ...
          if (!isReference(incomingValue)) {
            incomingFields = context.merge(incomingFields, incomingValue);
          }
        }
      }
    });
